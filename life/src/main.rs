...
fn census(_world: [[u8; 100]; 50]) -> u16 // Функция по подсчёту всех живых слеток
{
...
fn generation(world: [[u8; 100]; 50]) -> [[u8; 100]; 50] // 1 Итерация/Ход в мире
{
    let mut new_world = [[0u8; 100]; 50]; // создаём новый мир
...
fn main () // создаём мир
{
    // [0u8 <- u8 битные числа будут в массиве, также в массиве будут 50 чисел; 50 <- размер массива]
    let mut world = [[0u8; 100]; 50]; // игровая матрица, многомерный массив
    ...
    if args.len() < 2 { // Если не передали в консоли world, то генерируем world случайным образом
        for i in 0..49 { // присваиваем i = 0, и делаем условие i < 74; i++
    ...
    for _gens in 0..500 { // Делаем цикл на 50 поколений
        ...
        thread::sleep(time::Duration::from_millis(200)); // Делаем небольшую задержку между ходами поколений
    }
    ...
    // let new_world: [[u8; 50]; 50] = generation(world);
}

fn populate_from_file (filename: String) -> [[u8; 100]; 50] // Функция которая генерирует мир на основе файла пользователя
{
    let mut new_world = [[0u8; 100]; 50]; // создаём новый мир
    ...
    for i in 0..49 { // Делаем мир, в котором все ячейки мертвы
        for j  in 0..99 {
            new_world[i][j] = 0;
        }
    }
...
fn display_world(world: [[u8; 100]; 50]) // Отображаем нашу игру Жизнь на экране терминали/ консоли
{
    for i in 0..49 {
        for j in 0..99 {
            if world[i][j] == 1 {
